...
var GattCentral = require('./gatt_central');
var GattPeripheral = require('./gatt_peripheral');
var Gap = require('./gap');
var Hci = require('./hci');
...
var NobleBindings = function() {
  ...
  this._gatt_peripheral = new GattPeripheral(this._hci);
};

util.inherits(NobleBindings, events.EventEmitter);

NobleBindings.prototype.setServices = function(services) {
  this._gatt_peripheral.setServices(services);
...
NobleBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if (status === 0) {
    ...
    this._gatt_peripheral.setAclStream(aclStream);

    var gatt_central = new GattCentral(address, aclStream);

    this._gatts[uuid] = this._gatts[handle] = gatt_central;
